...
The application code is identical regardless of whether it deploys to bare metal or Azure (16). This document covers only the infrastructure: server setup, service configuration, release management, backup procedures, and monitoring integration. It implements the security standards (06) at the infrastructure level and integrates with background tasks (14) for worker and scheduler service definitions.
