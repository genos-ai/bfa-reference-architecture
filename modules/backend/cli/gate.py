...
def _cost_color(cost: float, budget: float) -> str:
    if budget <= 0:
        return "white"
    ratio = cost / budget
    if ratio > 0.9:
...
class CliGateReviewer:
    ...
    async def _review_pre_dispatch(self, ctx: GateContext) -> GateDecision:
        ...
        table.add_column("Task ID", style="cyan", width=24)
    ...
    async def _review_pre_layer(self, ctx: GateContext) -> GateDecision:
        ...
        table.add_column("Task ID", style="cyan", width=24)
        table.add_column("Agent", width=30)
        table.add_column("Input Keys", width=30)
        table.add_column("Instructions", ratio=1)
    ...
    def _header(self, ctx: GateContext, title: str) -> str:
        ...
        return (
            ...
            f"Cost: [{cost_color}]${ctx.total_cost_usd:.4f}[/{cost_color}]"
            f" / ${ctx.budget_usd:.2f}"
        )
